...
class App:
    ...
    def main_loop(self):
        ...
        while not self.quit:
            ...
            self.visuals(candidates, original)
        ...
        for round in self.tracked_objects:
            for obj in round:
                if str(obj) in summary:
                    summary[str(obj)]["occurance"] += 1
                    summary[str(obj)]["distances"].append(obj.distance)
                else:
                    summary[str(obj)] = {"occurance":1,"distances": [obj.distance] }

        for key in summary:
            summary[key]["distances"] = sum(summary[key]["distances"]) / len(summary[key]["distances"])
